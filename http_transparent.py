...
#Inject JS
#Rajoute le choix entre différents Payload
...
class ProxyRequestHandler(BaseHTTPRequestHandler):
	...
	def gen_openssl_cnf(self, filepath, hostname):
		conf = "[ req ]\n"
		conf += "default_bits = 2048\n"
		conf += "distinguished_name = req_distinguished_name\n"
		conf += "req_extensions = SAN\n\n"
		conf += "[ req_distinguished_name ]\n"
		conf += "commonName = %s\n\n" % hostname
		conf += "[ SAN ]\n"
		conf += "subjectAltName = @alt_names\n\n"
		conf += "[alt_names]\n"
		conf += "DNS.1 = %s" % hostname
	...
	def connect_intercept(self):
		...
		with self.lock:
			if not os.path.isfile(certpath):
				self.gen_openssl_cnf(confpath, hostname)
				epoch = "%d" % (time.time() * 1000)
				p1 = Popen(["openssl", "req", "-new", "-sha256", "-key", self.certkey, "-subj", "/CN=%s" % hostname, "-config", confpath], stdout=PIPE)
				p2 = Popen(["openssl", "x509", "-req", "-days", "360", "-CA", self.cacert, "-CAkey", self.cakey, "-set_serial", epoch, "-extensions", "SAN", "-extfile", confpath, "-out", certpath], stdin=p1.stdout, stderr=PIPE)
				p2.communicate()
		...
		conntype = self.headers.get('serve_Proxy-Connection', '')
